...
def test_interactive_delete_standalone_number_exact_y(wtk_bin, run_wtk, repo_factory) -> None:
    ...
    result = run_delete_pty(wtk_bin, repo, b"1\rY\r").assert_success()
...
@pytest.mark.parametrize("keys", [b"\r", b"1\rn\r"])
...
def test_interactive_delete_table_shows_dirty_updated_and_inline_non_deletable_reason(wtk_bin, run_wtk, repo_factory) -> None:
    ...
    assert linked.exists()
    assert "Select worktrees to delete by number. Branches are preserved." in result.output
    assert "worktree" in result.output
    assert "branch" in result.output
    assert "updated" in result.output
    assert "state" in result.output
    assert "head" in result.output
    assert "Enter numbers/ranges to delete" in result.output
...
def test_interactive_delete_invalid_selection_fails_fast(wtk_bin, run_wtk, repo_factory) -> None:
    repo = repo_factory.init_repo("repo")
    ...
    result = run_delete_pty(wtk_bin, repo, b"99\r", timeout=5.0)

    result.assert_failure()
    assert linked.exists()
    assert "selection number out of range" in result.output
...
def test_interactive_delete_coordinated_primary_cascades(wtk_bin, run_wtk, repo_factory) -> None:
    ...
    result = run_delete_pty(wtk_bin, primary, b"1\rY\r").assert_success()
...
def test_interactive_delete_coordinated_dirty_auxiliary_is_shown_before_confirmation(wtk_bin, run_wtk, repo_factory) -> None:
    ...
    result = run_delete_pty(wtk_bin, primary, b"1\rn\r").assert_success()
...
def test_interactive_delete_broken_coordinated_row_does_not_abort_healthy_delete(wtk_bin, run_wtk, repo_factory) -> None:
    ...
    result = run_delete_pty(wtk_bin, primary, b"1\rY\r", timeout=5.0).assert_success()
